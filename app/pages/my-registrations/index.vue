...
const { events,loading,error,loadEvents,getEventsByUser} = useEvents()
const {userRegistrations,loadRegistrationsByUser,getRegistrationByEventAndUser,removeRegistration } = useRegistrations()
...
const eventosUsuario = computed(() => {
  ...
  const resultado = getEventsByUser(user.value.id, userRegistrations.value )
  return resultado
})
...
const cancelarInscripcion = async (eventId) => {
  try {
    const registration =
      getRegistrationByEventAndUser( eventId, user.value.id)
